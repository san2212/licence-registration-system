...
include "session_header.php";
	...
	$db= mysqli_connect('localhost','id1279441_elrs_db','elrs_pass','id1279441_elrs_db') or die('could not connect');
  	$username = $_SESSION['username'];
	
$msg = array();
			
$query= mysqli_query($db,"SELECT * FROM form where fn = '$username'");
...
if($username == $row['fn'])
{
	...
	$msg['msg6'] = "<form method='post' action=''>
<p id='notify_layout2'>Choose your payment option below:<br>• You can pay the registration charge of amount <b>₹ 30/-</b> by online payments via. paytm<br>• If not possible you can pay it on exam date, Choose your choice below..<br><br>
<button name='btn1' id='pay_btn'>₹ Pay Now</button> <button name='btn2' id='pay_btn1' onclick='return confirm("Are you sure you want to proceed with the transaction");' >₹ Pay during exams</button></p></form>";
...
else if ($row['verify'] == 3 || $row['verify'] == 4)
	{
	$msg['msg6'] = "
...
if(isset($_POST['btn2']))
{
header("Location: direct_payment_validation.php");
}
?>
